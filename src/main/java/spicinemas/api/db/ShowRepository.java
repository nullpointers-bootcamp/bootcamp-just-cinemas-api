...
public class ShowRepository {
    ...
    public List<DBShow> getShowsByDateAndMovieId(Date date, int movieId) {
        ...
        return dslContext.select().from(DSL.table("SHOW")).where(DSL.field("DATE").eq(dateSql)).and(DSL.field("MOVIE_ID").eq(movieId)).fetchInto(DBShow.class);
    }


    public DBShow getShowdById(int id) {
        return dslContext.select().from(DSL.table("SHOW")).where(DSL.field("ID").eq(id)).fetchOneInto(DBShow.class);
    }
    ...
    public List<String> getSeatsBookedByShowId(int showId) {
        return dslContext.select(DSL.field("TS.SEAT_NUMBER")).from(DSL.table("TRANSACTION_SEATS TS"))
                ...
                .where(DSL.field("S.ID").eq(showId)).fetchInto(String.class);
    }
}
